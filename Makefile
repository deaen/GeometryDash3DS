...
APP_DESCRIPTION		:=  Geometry Dash 3DS
APP_AUTHOR			:= 	deaen
...
#LOGO				:=	meta/splash.bin

UNIQUE_ID			:=	0x9140F
PRODUCT_CODE		:=	CTR-RTP-GMD
...
MAKEROM_ARGS		:=	-elf "$(OUTPUT).elf" -rsf "$(RSF_PATH)" -banner "$(BUILD)/banner.bnr" -icon "$(BUILD)/icon.icn" -DAPP_TITLE="$(APP_TITLE)" -DAPP_PRODUCT_CODE="$(PRODUCT_CODE)" -DAPP_UNIQUE_ID="$(UNIQUE_ID)" -major $(VERSION_MAJOR) -minor $(VERSION_MINOR) -micro $(VERSION_BUILD)
